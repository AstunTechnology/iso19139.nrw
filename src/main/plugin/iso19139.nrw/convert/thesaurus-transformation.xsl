...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="to-iso19139.nrw-keyword-with-anchor">
    <xsl:call-template name="to-iso19139.nrw-keyword">
  ...
  <xsl:template name="to-iso19139.nrw-keyword-as-xlink">
    <xsl:call-template name="to-iso19139.nrw-keyword">
  ...
  <xsl:template name="to-iso19139.nrw-keyword-as-xlink-with-anchor">
    <xsl:call-template name="to-iso19139.nrw-keyword">
      <xsl:with-param name="withXlink" select="true()"/>
      <xsl:with-param name="withAnchor" select="true()"/>
    </xsl:call-template>
  ...
  <!-- Convert a concept to an ISO19139 keywords.
    ...
    -->
  <xsl:template name="to-iso19139.nrw-keyword">
    ...
    <xsl:apply-templates mode="to-iso19139.nrw-keyword" select=".">
  ...
  <xsl:template mode="to-iso19139.nrw-keyword" match="*[not(/root/request/skipdescriptivekeywords)]">
        ...
        <xsl:when test="$withXlink">
          ...
          <xsl:attribute name="xlink:href"
                         select="concat(
                                  ...
                                  if ($withAnchor) then '&amp;transformation=to-iso19139.nrw-keyword-with-anchor' else '')"/>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template mode="to-iso19139.nrw-keyword" match="*[/root/request/skipdescriptivekeywords]">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>

    <xsl:call-template name="to-md-keywords-nrw">
      <xsl:with-param name="withAnchor" select="$withAnchor"/>
      <xsl:with-param name="withThesaurusAnchor" select="$withThesaurusAnchor"/>
      <xsl:with-param name="listOfLanguage" select="$listOfLanguage"/>
      <xsl:with-param name="textgroupOnly" select="$textgroupOnly"/>
  ...
  <xsl:template name="to-md-keywords-nrw">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>
    ...
    <gmd:MD_Keywords>
      ...
      <xsl:copy-of
        select="geonet:add-thesaurus-info-nrw($currentThesaurus, $listOfLanguage[1],
                              $withAnchor, $withThesaurusAnchor,
                              /root/gui/thesaurus/thesauri, not(/root/request/keywordOnly))"/>
    </gmd:MD_Keywords>
  </xsl:template>

  <xsl:function name="geonet:add-thesaurus-info-nrw">
  ...
  <xsl:template name="to-iso19139.nrw-extent">
